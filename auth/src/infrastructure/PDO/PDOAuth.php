...
namespace auth\infrastructure\PDO;
...
class PDOAuth implements AuthRepositoryInterface
{
    ...
    {
        // Prépare la requête SQL pour récupérer un utilisateur
        ...
        if ($data) {
            // Crée l'objet User et assigne l'ID récupéré
            $user = new User(
                $data['email'],
                $data['password'],
                $data['role']
            );
            $user->setID($data['id']); // Assigner l'ID à l'objet utilisateur
            return $user;
        }
